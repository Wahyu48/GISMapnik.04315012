...
POSTGIS_TABLE = dict(
    ...
    password='12345',
    dbname='kelasgis',
    table='(select ST_Buffer(ST_Centroid(geom), 2) as geom, nama from faskes) as faskes')
LAYER_NAME = 'pantai'
...
#    line_symbolizer.stroke_width = 5.0
    ...
    label = mapnik.TextSymbolizer(mapnik.Expression('[nama]'), 'DejaVu Sans Book',7,mapnik.Color('green'
    ))
    label.halo_radius = 1
